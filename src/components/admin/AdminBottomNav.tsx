...
const NAV_ITEMS: { icon: React.ElementType; label: string; page: Page }[] = [
  { icon: Home, label: 'Home', page: 'admin-home' },
        ...
        {NAV_ITEMS.map(({ icon: Icon, label, page }) => {
          ...
          return (
            <button
              ...
              className={`flex-1 flex flex-col items-center justify-center pt-3 pb-4 gap-1
            ...
            >
              ...
              {active && <span className="absolute bottom-0 w-8 h-0.5 bg-emerald-500 rounded-full" />}
            </button>
          );
        })}
      </div>
